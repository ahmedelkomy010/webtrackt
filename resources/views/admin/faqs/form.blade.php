...
@section('title', $isEdit ? 'تعديل سؤال' : 'إضافة سؤال')

@section('content')
<div class="mb-6">
    <a href="{{ route('admin.faqs.index') }}" class="text-sm text-tract-600">← العودة</a>
    <h1 class="text-2xl font-bold mt-2">{{ $isEdit ? 'تعديل سؤال' : 'إضافة سؤال جديد' }}</h1>
</div>

<form method="POST" action="{{ $isEdit ? route('admin.faqs.update', $item) : route('admin.faqs.store') }}" class="space-y-6">
    ...
    @foreach (['ar' => 'العربية', 'en' => 'English', 'ur' => 'اردو'] as $lang => $langLabel)
        <div class="bg-white rounded-2xl border p-6 space-y-4">
            ...
            <div><label class="block text-sm font-medium mb-1">السؤال</label><input type="text" name="question_{{ $lang }}" value="{{ old('question_'.$lang, data_get($item->question, $lang, '')) }}" required class="w-full px-4 py-2.5 rounded-xl border"></div>
            <div><label class="block text-sm font-medium mb-1">الإجابة</label><textarea name="answer_{{ $lang }}" rows="4" required class="w-full px-4 py-2.5 rounded-xl border">{{ old('answer_'.$lang, data_get($item->answer, $lang, '')) }}</textarea></div>
        </div>
    @endforeach
